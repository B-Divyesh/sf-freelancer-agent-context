...
function footer(): string {
  return `<footer><p>Keep each client’s work in its own workspace.</p><nav aria-label="Footer"><a class="nav-link" href="/privacy">Privacy</a><a class="nav-link" href="/terms">Terms</a><a class="nav-link" href="/art-provenance">Art provenance</a><a href="https://sociobot.in" rel="external">Built by Param Factory <span class="sr-only">(external site)</span></a></nav><p class="build">v0.1.10</p></footer>`;
}
...
async function loadDownload(): Promise<void> {
  ...
  try {
    ...
    const macChoices = platform === 'macOS' && assets.length > 1
      ? `<div class="download-choices">${assets.slice(1).map(asset => `<a href="${escapeHtml(asset.browser_download_url)}" rel="external">Download ${/aarch64|arm64/i.test(asset.name) ? 'Apple silicon' : 'Intel'} build from GitHub <span class="sr-only">(external site)</span></a>`).join('')}</div>`
      : '';
    ...
    panel.innerHTML = `<p class="stamp">DETECTED · ${platform.toUpperCase()}</p><a class="button primary" href="${escapeHtml(assets[0].browser_download_url)}" rel="external">Download for ${platform} from GitHub <span class="sr-only">(external site)</span> ${icon('arrow')}</a><p>${escapeHtml(assets[0].name)}</p>${macChoices}${installNote}`;
  } catch {
    panel.innerHTML = `<p class="stamp">DETECTED · ${platform.toUpperCase()}</p><p>Downloads are being published.</p><a class="button secondary" href="https://github.com/${REPO}/releases" rel="external">Open the GitHub release page <span class="sr-only">(external site)</span> ${icon('arrow')}</a>`;
  }
}
